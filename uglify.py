...
def loadImages(path, batch_size, extension=".jpg"):
    """
    Returns np array of all files with the specified extension.
    :param string: path to folder 
    :param string: file type
...
def convert(target_image, originals, batch_size=32, iterations=100):
    """
    ...
    with tf.variable_scope('spatial_transformer'):
        ...
        optim = tf.train.AdamOptimizer(1e-05).minimize(loss)
